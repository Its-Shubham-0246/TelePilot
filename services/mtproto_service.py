...
class MTProtoService:
    ...
    async def check_account_owned_groups(self, session_str: str) -> Tuple[bool, str]:
        """
        Connects via MTProto, inspects joined dialogs/groups/channels,
        and returns lists of groups/channels owned (creator) or managed (admin) by the account.
        """
        if not session_str:
            return False, "Session token is invalid or empty."
        ...
        try:
            ...
            async for dialog in client.iter_dialogs():
                if dialog.is_group or dialog.is_channel:
                    ...
                    count_str = f"{participants_count:,}" if isinstance(participants_count, int) else "N/A"

                    # Group type icon
                    chat_type = "Channel" if dialog.is_channel else "Group"
                    icon = "📢" if dialog.is_channel else "👥"

                    group_info = f"  • {icon} <b>{title}</b> ({chat_type})\n    └ <b>ID:</b> <code>{chat_id}</code> | <b>Members:</b> {count_str}"
            ...
            summary = []
            summary.append(f"<b>👑 Owned Groups/Channels ({len(owned_groups)}):</b>")
            if owned_groups:
                summary.extend(owned_groups)
            else:
                summary.append("  <i>None — this account is not the owner/creator of any groups or channels.</i>")

            summary.append("")
            summary.append(f"<b>🛡️ Admin in Groups/Channels ({len(admin_groups)}):</b>")
            if admin_groups:
                summary.extend(admin_groups)
            else:
                summary.append("  <i>None — this account has no admin rights in other groups.</i>")

            summary.append("")
            summary.append(f"<b>💬 Regular Joined Groups:</b> {regular_groups}")

            return True, "\n".join(summary)

        except Exception as e:
            logger.error(f"[MTProto] check_account_owned_groups failed: {e}")
            return False, f"Failed to check group ownership: {e}"
        finally:
            try:
                await client.disconnect()
            except Exception:
