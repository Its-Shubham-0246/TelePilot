...
class JobLog(Base):
    ...
    account_id: Mapped[Optional[int]] = mapped_column(ForeignKey("telegram_accounts.id", ondelete="SET NULL"), nullable=True)
    target_chat: Mapped[str] = mapped_column(String(255), nullable=False)
    status: Mapped[str] = mapped_column(String(50), nullable=False)  # SUCCESS, FAILED, FLOOD_WAIT
